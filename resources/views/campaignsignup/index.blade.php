...
@section('breadcrumb')
    <ol class="breadcrumb">
        ...
        <li class="breadcrumb-item active" aria-current="page">Active Campagnes</li>
    </ol>
@endsection
...
@section('body')
    @include('layouts.templates.tableIndex', [
        'data' => (object) [
            'url' => 'campagnes',
            'results' => $campagnes,
            'terms' => ['campagne', 'Start Campaign', 'Omschrijving'],
            'columns' => ['#', 'Campagne', 'Start Campaign', 'Omschrijving'],
        ]
    ])
    @foreach ($campagnes as $campagne)
        <tr>
            ...
            @endif
             @if (!empty($search) && !empty($term) && $term=="created_at")
                <th>{!!$campagne->created_at = str_ireplace($search, "<mark><u>$search</u></mark>", $campagne->created_at)!!}</th>
            @else
                <th>{{$campagne->created_at}}</th>
            @endif
            {{-- Voornaam --}}
